...
const resolvers = {
    ...
    Mutation: {
        ...
        addCharacter: async (parent, { playbook, background, drive, origin, name, str, dex, int, wis, con, cha, maxHP, currentHP, damage, level, exp }, context) => {
            if (context.user) {
                const character = await Character.create({ playbook, background, drive, origin, name, str, dex, int, wis, con, cha, maxHP, currentHP, damage, level, exp, characterCreator: context.user.username });
                return character;
            }
        },
